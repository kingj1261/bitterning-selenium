...
import org.openqa.selenium.remote.server.rest.HandlerFactory;
...
public class JsonHttpCommandHandler {
  ...
  public JsonHttpCommandHandler(DriverSessions sessions, Logger log) {
    ...
    addUserDefinedCommands();
  }

  private void addUserDefinedCommands() {
    ServiceLoader<HandlerFactory> handlerFactoryServiceLoader=ServiceLoader.load(HandlerFactory.class);
    Iterator<HandlerFactory> iterator=handlerFactoryServiceLoader.iterator();
    while(iterator.hasNext()){
      HandlerFactory factory=iterator.next();
      addNewUsedDefinedMapping(factory.commandName(),factory);
    }
  }
